            ...
            let contentObjects = contents.reduce((acc, content, i) => {
              if (content) {
                ...
                if (content.trim().includes(" Conseils d’utilisation")) {
                  ...
                  content.trim().includes(" Caractéristiques techniques")
                ) {
                  key = "tech_charac";
                  ...
                  content.trim().includes("Configuration maximale admissible")
                ) {
                  let parts = content
                    .trim()
                    .split("Configuration maximale admissible");
                  if (parts.length > 1) {
                  ...
                  content
                    .trim()
                    .includes(
                      "Angle maximal d’orientation pour conformité arrêté "
                    )
                ) {
                  let parts = content
                    .trim()
                  ...
                  content.trim() === "*" ||
                  content.trim() === "Couleur" ||
                  content.trim().endsWith("00K")
                ) {
                  acc["range"]["Colors"] = acc["range"]["Colors"] || [];
                  acc["range"]["Colors"].push(content.trim());
                } else if (i === 4) {
                  key = "title";
